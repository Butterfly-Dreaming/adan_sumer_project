...
//    nRet = MV_CC_SetFloatValue(handle, "AcquisitionFrameRate", 120);
    ...
    nRet = MV_CC_SetFrameRate(handle, frame_rate);
    if(MV_OK != nRet)
    {
        printf("Set FrameRate faile, nRet [0x%d]!\n", nRet);
        return;
    }
    /**
     *@brief: 手动曝光
    ...
    nRet = MV_CC_SetFloatValue(handle, "ExposureTime", 5000);
    if (MV_OK != nRet)
    {
        printf("Set ExposureTime fail nRet [0x%d]\n!", nRet);
    }
